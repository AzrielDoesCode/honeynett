...
#host_key = "server.key"
host_key = paramiko.RSAKey(filename='server.key')  #Attention Secure this
...
def emulated_shell(channel, client_ip):
    channel.send(b'Welcome to the emulated shell nigger $\n')
    command = b""
    while True:
        char = channel.recv(1)
        channel.send(char)
        ...
        if char == b'\r':
            if command.strip() == b'exit':
                ...
                response = b'\b' + b"jumpbox1.conf" + b"\r\n"
        ...
        channel.send(b'Welcome to the emshel $\n')
        channel = b""
...
def honeypot(address, port, username, password):
    ...
    socks = socket.socket(socket.AF_INET, socket.SOCK_STREAM) #IPV4 and TCP Stateful socket establishment
    socks.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1) #Allow reuse of socket
